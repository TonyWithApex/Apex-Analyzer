...
// === Apex Password Gate (non-destructive) ===
(function(){
  const overlay = document.getElementById('login-overlay');
  const form = document.getElementById('login-form');
  const pw = document.getElementById('apex-password');
  const error = document.getElementById('login-error');
  if(!overlay || !form || !pw) return;
  // Keep existing password if set in localStorage
  const KEY='apex_unlocked';
  if(localStorage.getItem(KEY)==='yes'){
    overlay.style.display='none';
  }
  const PASSWORD = window.APEX_PASSWORD || 'apex'; // if your existing code defines APEX_PASSWORD, we respect it.
  form.addEventListener('submit', function(e){
    e.preventDefault();
    const ok = pw.value === PASSWORD;
    if(ok){
